...
def todo_list(request):
    ...
    now = timezone.now()
    upcoming_due_date = now + timedelta(days=1)  # Adjust as needed
...
def task_completion_pie_chart(request):
    # Querying completed and non-completed tasks
    ...
    colors = ['#4CAF50', '#FF6347']  # Green for completed, Red for not completed
    explode = (0.1, 0)  # Explode the first slice (completed)
    ...
    # Save the plot to a bytes buffer and serve as HTTP response
    buffer = BytesIO()
